...
class FenetreJeu extends JFrame implements ActionListener 
{
		...
		for(int i=0;i<5;i++){
			...
			if(GrilleVirtuelle[pos_x][pos_y]==listeBateau[i].getCode()){
				...
				if(listeBateau[i].getEtat()>0){
					MonLabel.setText(listeBateau[i].getNom() + " a été touché...");
				}else{
					MonLabel.setText(listeBateau[i].getNom() + " a été coulé...");					
				}
  ...
  private boolean verifWIN(){
  	...
  	for(int i=0;i<5;i++){
  		...
  		if(listeBateau[i].getEtat()>0){ gagne = false; }	
  	}
  ...
  private char[][] CensureGrille(){
  		...
  		for(int i=0;i<10;i++){
  			for(int j=0;j<10;j++){
  				if(GrilleVirtuelle[i][j]!='a' && GrilleVirtuelle[i][j]!='b' && GrilleVirtuelle[i][j]!='c' && GrilleVirtuelle[i][j]!='d' && GrilleVirtuelle[i][j]!='e'){
